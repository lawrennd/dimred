% DEMDAGSTUHLTALK Show demos for Dagstuhl talk in order.
...
disp('Ready ... Variational GP-LVM on stick man.')
disp('[void, connect] = mocapLoadTextData(''run1'');')
...
disp('Ready ... Variational GP-LVM on stick man (Linear).')
disp('[void, connect] = mocapLoadTextData(''run1'');')
disp('lvmResultsDynamic(''vargplvm'', ''stick'', 2, ''stick'', connect)')
r = input('Type ''R'' to run or ''S'' to skip: ', 's');
switch r
  case {'r', 'R'}
   close all
   ...
   [void, connect] = mocapLoadTextData('run1');
   % Load the results and display dynamically.
   lvmResultsDynamic('vargplvm', 'stick', 2, 'stick', connect)
 otherwise
end
