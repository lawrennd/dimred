...
if strcmp(gbox,'on');
  line(X(:),Y(:),'linestyle','-','linewidth',glinewidth,'color', ...
       gcolor,'tag','m_grid_box'); %,'clip','off');
end;
...
if ~isempty(xtick),
 ...
 if drawticks,
   ...
   line(reshape([ltx;NaN+ones(1,m)],(n+1)*m,1),reshape([lty;NaN+ones(1,m)],(n+1)*m,1),...
        'linestyle','-','color',gcolor,'linewidth',glinewidth,'tag','m_grid_xticks-lower');
   line(reshape([utx;NaN+ones(1,m)],(n+1)*m,1),reshape([uty;NaN+ones(1,m)],(n+1)*m,1),...
        'linestyle','-','color',gcolor,'linewidth',glinewidth,'tag','m_grid_xticks-upper');
 end;
...
if ~isempty(ytick),
 ...
 if drawticks,
   [n,m]=size(ltx);
   line(reshape([ltx;NaN+ones(1,m)],(n+1)*m,1),reshape([lty;NaN+ones(1,m)],(n+1)*m,1),...
        'linestyle','-','color',gcolor,'linewidth',glinewidth,'tag','m_grid_yticks-left');
   line(reshape([utx;NaN+ones(1,m)],(n+1)*m,1),reshape([uty;NaN+ones(1,m)],(n+1)*m,1),...
        'linestyle','-','color',gcolor,'linewidth',glinewidth,'tag','m_grid_yticks-right');
 end;
 ...
 for k=ik,
   [rotang(k), horizk, vertk] = upright(rotang(k), horiz, vert);
   text(xx(k),yy(k),labs{k},'fontsize',gfontsize*scl(k),'color',gcolor,...
...
patch(reshape(u2x(kk),size(kk,1),size(kk,2))',...
      ...
      repmat(bitmax  ,size(kk,2),size(kk,1)),'w','edgecolor','k','tag','m_grid_fancybox1');
patch(reshape(l2x(kk),size(kk,1),size(kk,2))',...
      reshape(l2y(kk),size(kk,1),size(kk,2))',...
      repmat(bitmax-1,size(kk,2),size(kk,1)),'k','tag','m_grid_fancybox1');
...
kk=kk+ones(size(kk,1),1)*[1 2:2:(dpatch*2+2) (dpatch*2+1):-2:3];
 patch(reshape(l2x(kk),size(kk,1),size(kk,2))',...
       ...
       'w','edgecolor','k','linewidth',.2,'tag','m_grid_fancybox2');
 patch(reshape(u2x(kk),size(kk,1),size(kk,2))',...
       reshape(u2y(kk),size(kk,1),size(kk,2))',...
       repmat(bitmax-1,size(kk,2),size(kk,1)),...
       'w','edgecolor','k','linewidth',.2,'tag','m_grid_fancybox2');
...
line(reshape(mean(l2x(:,kk)),k1,k2),reshape(mean(l2y(:,kk))',k1,k2),...
     repmat(bitmax,k1,k2),'color','k','tag','m_grid_fancybox2');
